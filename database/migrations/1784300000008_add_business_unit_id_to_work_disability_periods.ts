...
/**
 * USRH1784259058498 — periodos de incapacidad: marca propia con backfill de 2 saltos
 * (periodo → incapacidad → empleado). No filtrar padres soft-deleted.
 */
export default class extends BaseSchema {
    ...
    this.defer(async (db) => {
      ...
      await db.rawQuery(
        `ALTER TABLE \`${this.tableName}\`
         ...
         ADD INDEX \`work_disability_periods_business_unit_id_index\` (\`business_unit_id\`),
         ADD CONSTRAINT \`work_disability_periods_business_unit_id_foreign\`
  ...
  async down() {
    this.schema.alterTable(this.tableName, (table) => {
      table.dropForeign(['business_unit_id'], 'work_disability_periods_business_unit_id_foreign')
      table.dropIndex(['business_unit_id'], 'work_disability_periods_business_unit_id_index')
