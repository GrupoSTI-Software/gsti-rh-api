...
/**
 * Defensa en profundidad (ESB-07-08-03-08) — `employee_devices` (riesgo
...
export default class extends BaseSchema {
    ...
    this.defer(async (db) => {
      ...
      await db.rawQuery(
        `ALTER TABLE \`${this.tableName}\`
         ...
         ADD INDEX \`employee_devices_business_unit_id_index\` (\`business_unit_id\`),
         ADD CONSTRAINT \`employee_devices_business_unit_id_foreign\`
  ...
  async down() {
    this.schema.alterTable(this.tableName, (table) => {
      table.dropForeign(['business_unit_id'], 'employee_devices_business_unit_id_foreign')
      table.dropIndex(['business_unit_id'], 'employee_devices_business_unit_id_index')
