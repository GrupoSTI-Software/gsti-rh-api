...
/**
 * Defensa en profundidad (ESB-07-08-03-08) — `employee_assessments` (riesgo
 * Alto) es punto de entrada directo sin marca de pertenencia propia.
 */
export default class extends BaseSchema {
  protected tableName = 'employee_assessments'
    ...
    this.defer(async (db) => {
      ...
      await db.rawQuery(
        `ALTER TABLE \`${this.tableName}\`
         ...
         ADD INDEX \`employee_assessments_business_unit_id_index\` (\`business_unit_id\`),
         ADD CONSTRAINT \`employee_assessments_business_unit_id_foreign\`
  ...
  async down() {
    this.schema.alterTable(this.tableName, (table) => {
      table.dropForeign(['business_unit_id'], 'employee_assessments_business_unit_id_foreign')
      table.dropIndex(['business_unit_id'], 'employee_assessments_business_unit_id_index')
