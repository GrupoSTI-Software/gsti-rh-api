...
/**
 * Defensa en profundidad (ESB-07-08-03-08) — `employee_vacation_archives`
 * (riesgo Alto) es punto de entrada directo sin marca de pertenencia propia.
 * También es padre de `employee_vacation_archive_contents` (ver esa
 * migración).
 */
export default class extends BaseSchema {
    ...
    this.defer(async (db) => {
      ...
      await db.rawQuery(
        `ALTER TABLE \`${this.tableName}\`
         ...
         ADD INDEX \`employee_vacation_archives_business_unit_id_index\` (\`business_unit_id\`),
         ADD CONSTRAINT \`employee_vacation_archives_business_unit_id_foreign\`
  ...
  async down() {
    this.schema.alterTable(this.tableName, (table) => {
      table.dropForeign(['business_unit_id'], 'employee_vacation_archives_business_unit_id_foreign')
      table.dropIndex(['business_unit_id'], 'employee_vacation_archives_business_unit_id_index')
