...
/**
 * USRH1784259058510 — periodos de lactancia: marca propia desde la empleada.
 */
export default class extends BaseSchema {
    ...
    this.defer(async (db) => {
      ...
      await db.rawQuery(
        `ALTER TABLE \`${this.tableName}\`
         ...
         ADD INDEX \`employee_lactation_periods_business_unit_id_index\` (\`business_unit_id\`),
         ADD CONSTRAINT \`employee_lactation_periods_business_unit_id_foreign\`
  ...
  async down() {
    this.schema.alterTable(this.tableName, (table) => {
      table.dropForeign(['business_unit_id'], 'employee_lactation_periods_business_unit_id_foreign')
      table.dropIndex(['business_unit_id'], 'employee_lactation_periods_business_unit_id_index')
