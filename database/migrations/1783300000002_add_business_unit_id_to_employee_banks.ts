...
/**
 * Defensa en profundidad (ESB-07-08-03-08) — `employee_banks` (datos
 * bancarios, riesgo Crítico) es punto de entrada directo sin marca de
 * pertenencia propia.
 */
export default class extends BaseSchema {
    ...
    this.defer(async (db) => {
      ...
      await db.rawQuery(
        `ALTER TABLE \`${this.tableName}\`
         ...
         ADD INDEX \`employee_banks_business_unit_id_index\` (\`business_unit_id\`),
         ADD CONSTRAINT \`employee_banks_business_unit_id_foreign\`
  ...
  async down() {
    this.schema.alterTable(this.tableName, (table) => {
      table.dropForeign(['business_unit_id'], 'employee_banks_business_unit_id_foreign')
      table.dropIndex(['business_unit_id'], 'employee_banks_business_unit_id_index')
