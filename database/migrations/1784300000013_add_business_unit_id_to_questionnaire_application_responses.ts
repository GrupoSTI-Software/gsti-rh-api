...
/**
 * USRH1784259058521 — respuestas de cuestionario NOM-035: marca propia
 * llaveada a la APLICACIÓN, no al empleado (empleado prestado puede
...
export default class extends BaseSchema {
    ...
    this.defer(async (db) => {
      ...
      await db.rawQuery(
        `ALTER TABLE \`${this.tableName}\`
         ...
         ADD INDEX \`questionnaire_application_responses_business_unit_id_index\` (\`business_unit_id\`),
         ADD CONSTRAINT \`questionnaire_application_responses_business_unit_id_foreign\`
  ...
  async down() {
    this.schema.alterTable(this.tableName, (table) => {
      table.dropForeign(
        ['business_unit_id'],
        'questionnaire_application_responses_business_unit_id_foreign'
      )
      table.dropIndex(
        ['business_unit_id'],
        'questionnaire_application_responses_business_unit_id_index'
      )
