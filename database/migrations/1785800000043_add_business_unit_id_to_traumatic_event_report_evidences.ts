...
/**
 * USRH1786595131490 — marca de empresa en evidencias de evento traumático.
...
export default class extends BaseSchema {
    ...
    this.defer(async (db) => {
      ...
      await db.rawQuery(
        `ALTER TABLE \`${this.tableName}\`
         ...
         ADD INDEX \`traumatic_event_report_evidences_business_unit_id_index\` (\`business_unit_id\`),
         ADD CONSTRAINT \`traumatic_event_report_evidences_business_unit_id_foreign\`
  ...
  async down() {
    this.schema.alterTable(this.tableName, (table) => {
      table.dropForeign(
        ['business_unit_id'],
        'traumatic_event_report_evidences_business_unit_id_foreign'
      )
      table.dropIndex(
        ['business_unit_id'],
        'traumatic_event_report_evidences_business_unit_id_index'
      )
