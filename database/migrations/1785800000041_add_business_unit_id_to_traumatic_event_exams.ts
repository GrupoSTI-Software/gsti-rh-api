...
/**
 * USRH1786595131490 — marca de empresa en exámenes de evento traumático.
 * Hereda del reporte padre. No se filtra `traumatic_event_report_deleted_at`
 * (R-T1): un anexo cuyo reporte está borrado lógicamente también recibe marca.
 */
export default class extends BaseSchema {
    ...
    this.defer(async (db) => {
      ...
      await db.rawQuery(
        `ALTER TABLE \`${this.tableName}\`
         ...
         ADD INDEX \`traumatic_event_exams_business_unit_id_index\` (\`business_unit_id\`),
         ADD CONSTRAINT \`traumatic_event_exams_business_unit_id_foreign\`
  ...
  async down() {
    this.schema.alterTable(this.tableName, (table) => {
      table.dropForeign(['business_unit_id'], 'traumatic_event_exams_business_unit_id_foreign')
      table.dropIndex(['business_unit_id'], 'traumatic_event_exams_business_unit_id_index')
