...
/**
 * Defensa en profundidad (ESB-07-08-03-08) — `position_assessment_profiles`
...
export default class extends BaseSchema {
    ...
    this.defer(async (db) => {
      ...
      await db.rawQuery(
        `ALTER TABLE \`${this.tableName}\`
         ...
         ADD INDEX \`position_assessment_profiles_business_unit_id_index\` (\`business_unit_id\`),
         ADD CONSTRAINT \`position_assessment_profiles_business_unit_id_foreign\`
  ...
  async down() {
    this.schema.alterTable(this.tableName, (table) => {
      table.dropForeign(['business_unit_id'], 'position_assessment_profiles_business_unit_id_foreign')
      table.dropIndex(['business_unit_id'], 'position_assessment_profiles_business_unit_id_index')
