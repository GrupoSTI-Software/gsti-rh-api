...
/**
 * USRH1784259058510 — evidencias de lactancia: backfill 2 saltos vía periodo.
...
export default class extends BaseSchema {
    ...
    this.defer(async (db) => {
      ...
      await db.rawQuery(
        `ALTER TABLE \`${this.tableName}\`
         ...
         ADD INDEX \`employee_lactation_period_evidences_business_unit_id_index\` (\`business_unit_id\`),
         ADD CONSTRAINT \`employee_lactation_period_evidences_business_unit_id_foreign\`
  ...
  async down() {
    this.schema.alterTable(this.tableName, (table) => {
      table.dropForeign(
        ['business_unit_id'],
        'employee_lactation_period_evidences_business_unit_id_foreign'
      )
      table.dropIndex(
        ['business_unit_id'],
        'employee_lactation_period_evidences_business_unit_id_index'
      )
