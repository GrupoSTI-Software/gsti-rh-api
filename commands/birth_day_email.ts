...
export default class BirthDayEmail extends BaseCommand {
  static commandName = BIRTH_DAY_EMAIL_COMMAND
  ...
  async run() {
    this.logger.info('Inicio: felicitación de cumpleaños a empleados')
    const service = new EmployeeCelebrationEmailService()
    try {
      const result = await TenantContext.runUnscoped(
        () =>
          service.run(EMPLOYEE_CELEBRATION_EMAIL_KIND.BIRTHDAY_EMPLOYEE, {
            info: (m) => this.logger.info(m),
            error: (m) => this.logger.error(m),
            warning: (m) => this.logger.warning(m),
          }),
        EMPLOYEE_CELEBRATION_RUN_UNSCOPED_REASONS[EMPLOYEE_CELEBRATION_EMAIL_KIND.BIRTHDAY_EMPLOYEE]
      )
      ...
      this.logger.error(`Error fatal en felicitación de cumpleaños: ${message}`)
      this.exitCode = 1
    }
  }
}
