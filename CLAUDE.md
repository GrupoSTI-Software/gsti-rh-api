...
- Todo cambio de catálogo se verifica con `NODE_ENV=test DB_DATABASE=sae_pruebas node ace migration:fresh --seed` y después `NODE_ENV=test DB_DATABASE=sae_pruebas node ace permissions:check-consistency`, que debe terminar sin hallazgos (código de salida 0).
