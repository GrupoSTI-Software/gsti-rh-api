...
- `node ace test` fija `NODE_ENV=test` y lee `.env.test` por su cuenta; `node ace migration:*`, `db:seed` y los demás comandos no. Sin `NODE_ENV=test DB_DATABASE=valanserh_test` delante caen en la BD de `.env`. Otra BD u otro puerto personal van en `.env.test.local`, que no se versiona; quien lo use cambia también el nombre de la BD en estos comandos, porque una variable en línea gana sobre cualquier archivo `.env` y `DB_DATABASE=valanserh_test` ignoraría la de `.env.test.local`.
