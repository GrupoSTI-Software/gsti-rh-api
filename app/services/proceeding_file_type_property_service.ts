...
import { ProceedingFileTypePropertySystemSettingCategoryFilterInterface } from '../interfaces/proceeding_file_type_property_system_setting_category_filter_interface.js'
...
export default class ProceedingFileTypePropertyService {
  ...
  /**
   * Misma estructura que getCategories, pero los valores se filtran solo por proceedingFileId
   * (archivos ligados a system_setting, sin employeeId en la consulta de valores).
   */
  async getCategoriesBySystemSetting(
    filter: ProceedingFileTypePropertySystemSettingCategoryFilterInterface
  ) {
    const proceedingFileTypePropertyValueCategories = await ProceedingFileTypeProperty.query()
      .select(
        'proceedingFileTypePropertyCategoryName',
      ...
      .where('proceeding_file_type_id', filter.proceedingFileTypeId)
      .orderBy('proceedingFileTypePropertyCategoryName', 'asc')
    const categories: {
      [key: string]: {
        name: string
    ...
    for (const item of proceedingFileTypePropertyValueCategories) {
      ...
      if (proceedingFileTypeProperty) {
        const proceedingFileTypePropertyValues = await ProceedingFileTypePropertyValue.query()
          .whereNull('proceeding_file_type_property_value_deleted_at')
          .where('proceedingFileId', filter.proceedingFileId)
          .where(
            'proceedingFileTypePropertyId',
            proceedingFileTypeProperty.proceedingFileTypePropertyId
          )
