...
test.group('POST /api/platform/alliance-attributions — alta', (group) => {
  ...
  test('CA-1: hereda 15 % y 12 periodos y queda viva', async ({ client, assert }) => {
    ...
    assert.equal(data.allianceAttributionAccruedPeriods, 0)
    assert.equal(data.allianceAttributionRemainingPeriods, 12)
    assert.isTrue(data.allianceAttributionIsAccruing)
    assert.isNull(data.allianceAttributionNotAccruingReason)
    assert.equal(data.allianceAttributionAccruedAmountCents, 0)
