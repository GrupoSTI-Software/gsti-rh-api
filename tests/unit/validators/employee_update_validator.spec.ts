 ...
 * validador de edición. Estas llamadas no envían `employeeBusinessEmail`,
 * así que la regla `unique` de ese campo no corre y no hace falta BD.
 */
const base = {
...
test.group('updateEmployeeValidator — estructura opcional al editar (USRH1788466831270)', () => {
  test('sin departmentId ni positionId pasa y no inventa las claves', async ({ assert }) => {
    const data = await updateEmployeeValidator.validate(base, { meta: { employeeId: 0 } })
  ...
  test('0 sigue rechazado: no es "sin asignar", es un id inválido', async ({ assert }) => {
    await assert.rejects(() =>
      updateEmployeeValidator.validate({ ...base, departmentId: 0 }, { meta: { employeeId: 0 } })
    )
    await assert.rejects(() =>
      updateEmployeeValidator.validate({ ...base, positionId: 0 }, { meta: { employeeId: 0 } })
    )
  })
})
